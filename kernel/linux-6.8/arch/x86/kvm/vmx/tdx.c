...
void (* volatile vcpu_run_end_hook)(struct OutData *) = 0;
EXPORT_SYMBOL_GPL(vcpu_run_end_hook);
...
fastpath_t tdx_vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	if (vcpu_run_beg_hook) {
		struct kvm_tdx *t = to_kvm_tdx(vcpu->kvm);
		struct OutData d = {
			vcpu,
			//(void *)vcpu->arch.mmu->root.hpa,
			(void *)&vcpu->kvm->arch.tdp_mmu_roots,
			vcpu->vcpu_id,
			tdx->exit_reason.full,
			tdx->tdvmcall.rcx,
			(uint64_t *)(void*)&vcpu->arch,
			(void*) &tdx->pi_desc,
			POSTED_INTR_VECTOR,
			t->tdr_pa,
			(uint64_t)t->tdcs_pa,
			...
			tdx->tdvpr_pa,
			(uint64_t *)tdx->tdvpx_pa
		};
		vcpu_run_beg_hook(&d);
		//if (d.d) {
		//	tdx_deliver_interrupt(vcpu->arch.apic, 0,0,d.doo);
		//}
	}
...
void (* volatile td_init_hook)(struct td_params *, void *) = 0;
EXPORT_SYMBOL_GPL(td_init_hook);
...
#define MSG_ALLPKG	"All packages need to have online CPU to create TD. Online CPU and retry.\n"
    ...
    if (td_init_hook) td_init_hook(td_params, kvm);
